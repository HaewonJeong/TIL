package com.ohgiraffers.statemanagement.section03;
...
public class AuthApiServlet extends HttpServlet {
    ...
    private final ObjectMapper mapper = new ObjectMapper(); //Mapper 사용을 위해 Jackson Databind dependency build.gradle에 추가
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        sendError(resp, HttpServletResponse.SC_NOT_FOUND, "Unknow AP Path");
    }
    ...
    private void login(HttpServletRequest req, HttpServletResponse resp) throws IOException {
        ...
        if (!"user01".equals(loginRequest.getId()) || !"pass01".equals(loginRequest.getPassword())) {
            resp.setStatus((HttpServletResponse.SC_UNAUTHORIZED)); //id or pass가 아니면
    ...
    private void sendError(HttpServletResponse resp, int status, String message) throws IOException {
        //status랑 message만 전달하면 JSON으로 만들어서 응답할 수 있게 만들어줌
    ...
    //프론트에게 '현재 로그인 상태인지'와 로그인 했다면 사용자 ID가 무엇인지 알려주는 응답을 위한 클래스
    //front는 사용자 id을 보고 zstand에서 정보를 확인 가능
    ...
    //에러 응답용 객체 (에러 메세지를 Json로 감싸서 반환하기 위한 클래스)
