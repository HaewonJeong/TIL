...
def n_zeros():
    print("== np.zeros() ==")
    a = np.zeros(5) # 정수 0처럼 보여도 기본 dtype은 float이다
    print("1차원: ", a)
...
def n_linspace():  # 일정한 개수(num)로 균등 분할
    print("== np.linspace() ==")
    print("linspace(0, 1, 5) :", np.linspace(0, 1, 5))  # 0부터 1까지를 같은 간격 5개 숫자로 나눈다.
    print("linspace(0, 10, 6): ", np.linspace(0, 10, 6))

def compare_arange_linspace():
    print("arange는 간격(step) 기준", np.arange(0, 12, 2))
