...
class ProductCreate(BaseModel):
    model_config = ConfigDict(extra="forbid") # 없는 필드를 조용히 버리지 않고 오류로 처리
...
async def read_item(
    item_id: Annotated[int, Path(ge=1, le=9999)],  #Path 제약 조건
    keyword: Annotated[
        str | None,
        Query(alias="item-query", min_length=2, max_length=20) # 쿼리 스트링 제약조건
    ] = None
) -> dict: 
