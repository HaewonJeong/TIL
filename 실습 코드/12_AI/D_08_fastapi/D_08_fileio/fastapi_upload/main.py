...
ALLOWD_CONTENT_TYPES = {"text/plain" : ".txt", "application/pdf" : ".pdf"} #text/plain이면 .txt를, application/pdf면 .pdf를 들고 온다.
...
@app.post("/files", response_model=UploadResponse, tags=["multipart"])
async def inspect_upload(
    ...
    if content_type not in ALLOWD_CONTENT_TYPES:
        raise HTTPException(status_code=415, detail="text 또는 pdf만 허용합니다.")

    content = await file.read(MAX_FILE_SIZE + 1) #읽을때까지 기다려
...
async def upload_file(
    ...
    suffix = ALLOWD_CONTENT_TYPES.get(content_type)
