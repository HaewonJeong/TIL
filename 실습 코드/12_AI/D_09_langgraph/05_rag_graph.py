...
DOCUMENTS = [
    ...
    "올림픽공원은 야외 활동에 적합하며 입장료는 무료이다."
]

DEMO_QUESTIONS = [
    ("기본", "비 오는 날 송파에서 갈 만한 장소와 입장료는?")
    ("비교", "무료로 산책 할 수 있는 장소는?")
...
def build_grapgh():
    ...
    def generate(state: RagState) -> dict:
        ...
        context = "\n".join(f"-{text} for text in state["context"])
        prompt = (
...
def main() -> None:
    ...
    parser.add_argument("--run", action="store_ture", help="실제 Enbedding과 LLM을 호출")
