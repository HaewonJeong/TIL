...
def main() -> None:
    ...
    for rank, document in enumerate(retrieved_docs, start=1):
        print(
            ...
            f"line={document.metadata.get('line')}\n"
            f"{document.page_content}"
        )

    prompt_value = PROMPT.invoke({"context": context, "question": args.question})
    # print("\n",prompt_value)
    model = ChatOpenAI(model="gpt-4o-mini", temperature=0, max_tokens=300) # 사용자 질문+retrivel 질문 합쳐서 LLM에 던짐
...
# from vectorstore_common import load_environment, open_chroma_store
# # 외부에 있는것을 text에 넣고, 근거에 대한 질문을 한다.
...
#         "--rebuild-index",
#         action="store_ture",
#         help = "기존 Chroma collection을 초기화하고 문서를 다시 Embedding 합니다."
#     )
#     args = parser.parse_args()
#     if args.top_k < 1:
#         parser.error("--top-k는 1 이상이어야 합니다.")
