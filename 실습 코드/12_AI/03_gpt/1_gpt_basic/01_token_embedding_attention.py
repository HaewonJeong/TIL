...
def softmax(scores):
    ...
    exp_socres = np.exp(shifted) # exp() 각 원소를 지수값으로 변환
    return exp_socres / exp_socres.sum()
...
def gpt_embedding():
    ...
    print("바나나 벡터:", banana)
    print("자동차 벡터:", car)
    print("사과 VS 바나나 : ", consine_similarity(apple, banana))
...
def gpt_attention():
    ...
    # query는 지금 관심있는 정보
    query = np.array([1.0, 0.0])    # 나는 저런 사람을 찾고싶어정도로 생각
    ...
    # value는 attention결과로 실제 가져올 정보를 의미
    values = np.array([
        [10.0, 0.0],
        [8.0, 2.0],
        [0.0, 10.0]
...
def gpt_next_token():
    ...
    for token, probability in zip(candidates, probabilities):
