# 답변의 결과를 사용자가 편한형태말고
# 프로그램이 바로 처리할수 있도록 JSON 데이터로 변경
...
# Feedback을 Dictionay 타입으로 여러개 넣어놓음
FEEDBACKS = [
    {
        "id": 1,
    ...
    {
        "id": 3,
        "text": "내용은 어려운데 예제를 좀 더 연습해보고 싶습니다."
    }
]
...
def build_free_text_prompt() -> str:
    ...
    feedback_text = "\n".join(
        f"- {item['text']}" for item in FEEDBACKS
    )   
...
def build_json_prompt() -> str:
    ...
    return f"""
    너는 교육센터 수강평을 분석하는 데이터 분류기이다.
    아래 <feedbacks> 안의 JSON 배열을 분석하라

    규칙:
    - 반드시 순수 JSON 객체만 출력한다.
    - 최상위 키는 "items"하나만 사용한다.
    - 각 item은 id, sentiment, score, key_reason 필드만 가진다.
    ...
    {{
        "items": [
            {{
                ...
                "sentiment": "mixed",
                "score":75,
                "key_reason": "어려움은 있지만 실습이 이해해 도움이 된다는 평가입니다."
            }}
        ]
    }}
    <feedbacks>
