...
def get_gpt_response(client, prompt: str) -> str:
    """
    OPEN API를 호출하고 응답을 받아온다.
    """
    try:
        response = client.chat.completions.create(
            model = "gpt-4o-mini",
            messages = [
                {"role": "user", "content":prompt} # Role : user, assitant, system
            ],
            temperature=0.7
        )
...
def run_experiment():
    ...
    if not os.getenv("OPENAI_API_KEY"):
        print("[오류] OPEN_API_KEY가 존재하지 않습니다. .env 파일을 설정해 주세요.")
        return
    ...
    print(f"-> 전송할 프롬프트\n'{bad_prompt}")
    print("\n[AI 답변 대기 중...]")
    bad_respones = get_gpt_response(client, bad_prompt)
    print(f"\n[bad_respones_AI 답변 결과]:\n{bad_respones}\n")
    ...
    good_prompt = build_prompt(
        ...
        constraints = [
            ...
            "수식보다는 단어 간의 관련성이라는 직관적 원리를 기술할 것",
