...
def run_fewshot_experiment():
    if not os.getenv("OPENAI_API_KEY"):
        print("[오류] OPEN_API_KEY가 존재하지 않습니다. .env 파일을 설정해주세요.")
        return

    client = openai.OpenAI(api_key=os.getenv("OPENAI_API_KEY"))
    test_cases = [
        "진도가 너무 많은데 뇌정지가 오고 있어", # ACADEMIC (어려운 진도 호소)
        "출석 큐알코드 안 찍히는 수동체크 요망", # ADMIN (행정 및 출석 시스템 오류)
        "강사님 오늘은 커피가 필요한데 간식은 없을까요", # CHAT (단순 대화)
        "강의실 뒤쪽 에어콘 온도 좀 올려주세요. 너무 추워요" # ADMIN (시설 및 인프라)
    ]

    # 1. Zero-shot 정의 (예시 없이 규칙만 전달)
    zero_shot_template = """
    너는 교육 센터의 질문 분류기다. 수강생의 피드백 문장을 아래 3가지 카테고리 중 하나로 정확하게 분류해라.
    ...
    # 2. Few-shot 프롬프트 정의
    few_shot_template = """
    너는 교육 센터의 질문 분류기다. 수강생의 피드백 문장을 아래의 3가지 카테고리중 하나로 정확하게 분류해라.
    - ACADEMIC : 진도, 소스코드, 오류, 에러 질문
    - ADMIN : 행정 문의, 출석 QR 오류, 강의실 냉난방 등 시설 문의
    - CHAT : 일상대화, 친근한 농담, 인사
    결과는 오직 카테고리 단어('ACADEMIC', 'ADMIN', 'CHAT') 만 출력
