...
def create_model():
    ...
    if not os.getenv("OPENAI_API_KEY"):
        raise ValueError("OPEN_API_KEY가 없습니다. .env 파일을 확인하세요.")

    return ChatOpenAI(model="gpt-4o-mini", temperature=0, max_tokens=300)

def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--run", action="store_true", help="Open API를 호출하셨습니다.")    
    ...
    for user_message in DEMO_MESSAGES:
        # 현재 메세지의 profile 정보를 먼저 추출해야 현재 답변에도 반영된다.
        ...
        if model:
           ...
           reply = "[DRY RUN 응답 : 실제 모델을 호출하지 않는다.]"
