...
# python 01_no_rag_vs_rag.py --question "수업 시작 24시간 이내에 취소하면 얼마나 환불되나요?"
...
def build_prompts(question: str, context: str) -> tuple[str, str]: ## 결과를 튜플로 반환
...
def call_openai(no_rag_prompt: str, rag_prompt: str) -> None:
    ...
    model = ChatOpenAI(
        model="gpt-4o-mini",
        temperature=0.0,  #1에 가까울 수록 창의력이 높다
        max_tokens=300
    )
...
def main() -> None:
    ...
    parser.add_argument("--question", default=DEFAULT_QUESTION) ## question을 붙이면 원래 질문 말고 다른 질문을 할 수 있다.
    parser.add_argument(
        "--run", ## 실제 API 요청 선언
        action="store_true",
        help="실제 OpenAI를 호출합니다."
    )
    ...
    results = retrieve(index, args.question, top_k=3) ## 자른 것을 3개만 들고옴
    ...
    for result in relevant_results:
        ...
        if args.run:
            ...
            print("아래 RAG 프롬프트가 모델에 전달됩니다.")
            print(rag_prompt)
            print("실제 비교 실행: python ./no_rag_vs_ray.py --run")
