...
TEST_CASES = [
    ...
    ("VectorScore는 어떤 역할을 하나요?", "course_guide.txt"),
    ("OpenAI API 키는 어디에 저장하나요?", "lab_manual.txt"),
    ("와이파이 연결 문제는 어떻게 해결하나요?", "lab_manual.txt"),
    ("강의 포기 시 낸 돈을 돌려받는 조건은?", "center_policy.txt"),
...
def evaluate(documents: list[dict], setting: dict) -> None:
    ...
    for question, expected_source in TEST_CASES:
        ...
        if results:
            ...
            print(
                f"[{mark}] {question}\n"
                f"        기대={expected_source}, "
                f"검색={top_result['source']}, "
                f"score={top_result['score']:.3f}"
            )
        else:
            print(
                f"[실패] {question}\n"
                f"        기대={expected_source}, 검색 결과 없음"
            )
    ...
    print(
        f"Hit@{setting['top_k']}: "
        f"{hit_count}/{len(TEST_CASES)} = {hit_rate:.0%}"
    )
