...
def load_documents(data_dir: Path) -> list[dict]:
    ...
    for path in sorted(data_dir.glob("*.txt")):
        ...
        documents.append({ 
            "source": path.name,  # "source"는 메타데이터
            "text":text # "text"는 실제 문서 내용
        }) 
    return documents
...
def split_text(
    ...
    """문장을 묶어 chunk를 만들고 이전 문장 일부를 다음 chunck에 겹친다."""
    if chunk_size <= 0: #Chunk size 및 분리
        raise ValueError("chunk_size는 1이상이어야 합니다.")
    if overlap_sentences < 0:
        raise ValueError("overlap_sentence는 0 이상이어야 합니다.")
    
    sentences = split_sentences(text) # 정규 표현식으로 자름
    chunks = []
    current = []

    for sentence in sentences:  # Chunk 크기 만큼 잘라서 집어 넣음
    ...
    if current:
        chunks.append(" ".join(current))
    return chunks    # 결과 크기 만큼 chunk 크기를 전달

def tokenize(text: str) -> list[str]:
    words = re.findall(r"[A-Za-z0-9]+|[가-힣]+", text.lower()) # 정규 표현식에 대한 것을 찾는다. #안에 있는 것을 text lower로 바꾼다.
    return [word for word in words if word not in STOPWORDS] # words에 있는 단어 중에서 STOPWORDS에 없는 단어만 골라서 반환
...
def build_index(
    ...
    for document in documents:
        chunks = split_text(document["text"], chunk_size, overlap_sentences) # chunk size로 위에 있는것부터 1~5로 해서 indexingㅎ ㅏㄴ다.
...
def retrieve(index: list[dict], question: str, top_k: int = 3) -> list[dict]:
    ...
    for item in index: # Chunk로 잘라진 애들을 점수로 붙임
