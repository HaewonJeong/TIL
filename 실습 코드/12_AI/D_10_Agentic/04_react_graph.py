...
이번에는 Tool 결과가 부족할 때 다음 Action을 바꾸는 걸 반복

Action 1: strict 검색
 Observation 1: NOT_FOUND
Action 2: broad 검색으로 조건 수정
 Observation 2: FOUND
...
CLARIRICATION_QUESTION = "3명 여행 예산을 계산해줘"

class DestinationSearchInput(BaseModel):
    condition: str= Field(description="사용자가 원하는 여행 장소 조건")
    search_mode: Literal["strict", "board"] = Field(
        description=(
            "첫 검색은 strict, strict 결과가 NOT_FOUND 이면 핵심 조건으로 board 재검색"
        )
    )
...
def destination_lookup(
    ...
    broad_matches = [
        places for keyword, places in exact_data.items() # exact_data의 키:벨류 전달
...
def build_agent_graph():
            ...
            [
                SystemMessage(
                    content=(
                        ...
                        "2. strict Observation이 NOT_FOUND 이면 retry_hint를 읽고 핵심 "
                        "조건 하나로 search_mode='broad'를 한 번 호출한다.\n"
                        "3. broad도 NOT_FOUND 이면 더 호출하지 말고 사용자에게 조건"
                        "변경을 요청한다. \n"
    ...
    # tools_condition은 성공 여부를 판단하지 않는다. 마지막에 AIMessage에 tool_calls가 있으면
    # tools로, 없으면 END로 보내는 역할
...
def main() -> None:
    ...
    if not args.run:
        print("\n--run을 붙이면 실제 Action과 Observation 변화를 확인 할 수 있다.")
        return
