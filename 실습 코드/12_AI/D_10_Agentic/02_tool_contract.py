"""
일반 함수는 실행할수 있지만 모델이 사용방법을 알 수 없다.
...
def describe_tool(selected_tool) -> None:
    """모델에게 전달되는 Tool 계약의 세 요소를 출력"""
    print(f"\nTool 이름: {selected_tool.name}")
    print("Tool 설명: ", selected_tool.description)
...
def main() -> None:
    ...
    print_schema_error(
        "필수 인자 누락",
        {"travelers": 3, "ticket_price": 35000, "ticket_budget": 20000}
    )

    print("\n[5. 함수내부]")
    try:
        estimate_day_budget.invoke(
            {"travelers": 0, "ticketprice": 35000, "meal_budget": 20000}
        )
    except ValueError as error:
