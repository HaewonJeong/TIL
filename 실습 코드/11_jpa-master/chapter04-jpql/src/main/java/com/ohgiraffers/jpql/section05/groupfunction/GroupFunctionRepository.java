...
import jakarta.persistence.EntityManager;import jakarta.persistence.PersistenceContext;import org.springframework.stereotype.Repository;
...
public class GroupFunctionRepository {
    ...
    public long countMenuOfCategory(int categoryCode){
        //COUNT 사용 -> 특정 카테고리에 등록 된 메뉴 수 조회
    ...
    public List<Object[]> selectByGroupByHaving(long minPrice){
        String jpql =
                "SELECT m.categoryCode, SUM(m.menuPrice) FROM Section05Menu m "
                        + "GROUP BY m.categoryCode "
                        + "HAVING SUM(m.menuPrice) >= :minPrice";

        List<Object[]> sumPriceCategoryList = entityManager.createQuery(jpql)
